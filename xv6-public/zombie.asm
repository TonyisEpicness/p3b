...
main(void)
{
  ...
  1d:	6a 05                	push   $0x5
  1f:	e8 6f 03 00 00       	call   393 <sleep>
  24:	83 c4 10             	add    $0x10,%esp
  exit();
  27:	e8 d7 02 00 00       	call   303 <exit>
  2c:	66 90                	xchg   %ax,%ax
  2e:	66 90                	xchg   %ax,%ax

00000030 <test_and_set>:
#include "stat.h"
#include "fcntl.h"
#include "user.h"
#include "x86.h"
...
int test_and_set(int *old_ptr, int new_ptr) {
  ...
  *old_ptr = new_ptr;
  ...
  int old = *old_ptr;
  39:	8b 10                	mov    (%eax),%edx
  *old_ptr = new_ptr;
  3b:	89 08                	mov    %ecx,(%eax)
  return old;
}
  3d:	5d                   	pop    %ebp
...
void lock_acquire(lock_t *lock){
  ...
  76:	83 38 01             	cmpl   $0x1,(%eax)
  79:	75 05                	jne    80 <lock_acquire+0x10>
  7b:	eb fe                	jmp    7b <lock_acquire+0xb>
  7d:	8d 76 00             	lea    0x0(%esi),%esi
  80:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
    ;
}
  86:	5d                   	pop    %ebp
  87:	c3                   	ret    
...
free(void *ap)
{
 ...
 611:	a1 48 0b 00 00       	mov    0xb48,%eax
{
 ...
 651:	5b                   	pop    %ebx
  freep = p;
 652:	89 15 48 0b 00 00    	mov    %edx,0xb48
}
 658:	5e                   	pop    %esi
 659:	5f                   	pop    %edi
 65a:	5d                   	pop    %ebp
 ...
 689:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 68c:	89 15 48 0b 00 00    	mov    %edx,0xb48
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 6a9:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 6ac:	8b 3d 48 0b 00 00    	mov    0xb48,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 6ef:	73 4f                	jae    740 <malloc+0xa0>
    ...
    if(p == freep)
 6f1:	8b 3d 48 0b 00 00    	mov    0xb48,%edi
 ...
 71a:	e8 f1 fe ff ff       	call   610 <free>
  return freep;
 71f:	8b 15 48 0b 00 00    	mov    0xb48,%edx
 ...
 74c:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
 74f:	89 15 48 0b 00 00    	mov    %edx,0xb48
}
 ...
 75f:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
 760:	c7 05 48 0b 00 00 4c 	movl   $0xb4c,0xb48
 767:	0b 00 00 
    base.s.size = 0;
 76a:	bf 4c 0b 00 00       	mov    $0xb4c,%edi
    base.s.ptr = freep = prevp = &base;
 76f:	c7 05 4c 0b 00 00 4c 	movl   $0xb4c,0xb4c
 776:	0b 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 779:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 77b:	c7 05 50 0b 00 00 00 	movl   $0x0,0xb50
 782:	00 00 00 
    if(p->s.size >= nunits){
